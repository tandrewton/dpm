...
for t_stress in ${t_stress_arr[@]}; do
  for att in ${att_arr[@]}; do
    for om in 0.01; do
      for kl in 1.0; do
        for ka in 1.0; do
            echo bash bash/epi2D/submit_laserAblation.sh 50 30 3 1.15 0.94 0.85 $kl $ka 0.01 $att $om 4.0 4.0 4.0 1.0 0.0 $t_stress 0 1 1000 pi_ohern,day,scavenge 0-24:00:00 $numSeeds 1 >> joblist_PS.txt
        done
      done
    done
  done
