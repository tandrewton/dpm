...
numSeeds=1
calA0=(1.05)
att_arr=(0 0.001 0.01 0.05 0.1)
v0_arr=(0.05)
kecm_arr=(1.0 10.0 100.0)
koff_arr=(10.0 100.0 1000.0)
rm joblist_psm_att_v0_koff.txt
for att in ${att_arr[@]}; do 
  for v0 in ${v0_arr[@]}; do
    for koff in ${koff_arr[@]}; do
        for kecm in ${kecm_arr[@]}; do
            echo bash bash/cells/submit_psm.sh 40 20 $calA0 0.9 $att 0.0 $v0 50.0 $kecm $koff 1000 pi_ohern 0-1:45:00 $numSeeds 1 >> joblist_psm_att_v0_koff.txt
        done
    done
  done
done
...
bash bash/cells/submit_psm.sh 40 20 1.05 0.9 0 0.0 0.05 50.0 1.0 10.0 1000 pi_ohern,day 0-1:00:00 1 1
...
att_arr = ["0" "0.001" "0.01" "0.05" "0.1"];
v0_arr = ["0.05"];
k_ecm_arr = ["1.0" "10.0" "100.0"];
k_off_arr = ["10.0" "100.0" "1000.0"];
