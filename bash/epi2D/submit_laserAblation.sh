...
# name strings 
basestr=ablate_A0"$calA0"_t_stress"$t_stress"k_l"$kl"_k_a"$ka"_k_b"$kb"_w_ps"$strainRate_ps"_dsq"$deltaSq"_k_ps"$k_ps"_k_lp"$k_lp"_d_flag"$d_flag"_tau2"$tau2"_sm"$smooth"
...
for seed in `seq $startSeed $numSeedsPerRun $endSeed`; do
    ...
    for ss in `seq 0 $ssMax`; do
        ...
        # append to runString
        runString="$runString ; ./$binf $NCELLS $NV $ndelete $calA0 $phiMin $phiMax $kl $ka $kb $att $strainRate_ps $deltaSq $k_ps $k_lp $tau_lp $d_flag $t_stress $tau2 $smooth $seed $duration $outFileStem"
    done
