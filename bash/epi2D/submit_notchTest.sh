...
binf=bin/"$runstr".o
mainf=$maindir/notchTest.cpp

echo Running notchTest simulations with parameters:
echo NCELLS = "$NCELLS"
echo NV = "$NV"
echo calA0 = "$calA0"
echo phiMin = "$phiMin"
...
for seed in `seq $startSeed $numSeedsPerRun $endSeed`; do
    ...
    for ss in `seq 0 $ssMax`; do
        ...
        voidf=$simdatadir/$filestr.void

        # append to runString
        runString="$runString ; ./$binf $NCELLS $NV $calA0 $phiMin $phiMax $kl $att $B $Dr0 $strain $strainRate $runseed $duration $posf $energyf $stressf $voidf $loadingType"
    done
