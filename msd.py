        ...
        for k_ecm in k_ecm_arr:
            for k_off in k_off_arr:
                for att in att_arr:
                    ...
                    cellPos = cellPos[0 : len(cellPos) // 10]
                    msdtime, msd = calculate_msd(cellPos, time, len(cellPos) // 4)
                    ...
                    for i in range(xCoords.shape[1]):
                        axs[1, 0].plot(xCoords[:, i], yCoords[:, i], "-", linewidth=2)
                    ...
                    cellNeighborList = [dict() for x in range(len(cellPos))]
                    edgeLengthsList = []
                    ...
                    for timeii in range(0, len(cellPos)):
                        ...
                        cells = pyvoro.compute_2d_voronoi(
                            ...
                            [
                                [
                                    min(xCoords[timeii, :]) - 1,
                                    max(xCoords[timeii, :]) + 1,
                                ],
                                [
                                    min(yCoords[timeii, :]) - 1,
                                    max(yCoords[timeii, :]) + 1,
                                ],
                            ],  # limits
                            2.0,  # block size
                        )
                        ...
                        for i, cell in enumerate(cells):
                            ...
                            cellNeighbors[i] = {
                                j["adjacent_cell"] for j in cell["faces"]
                            }
                            ...
                            for nonAdjCellID in cellNotNeighbors:
                                if nonAdjCellID != i:
                                    ...
                                    if neighborLifetimesMatrix[i][nonAdjCellID] > 0:
                                        ...
                                        cellPos2 = np.array(
                                            [
                                                xCoords[timeii][nonAdjCellID],
                                                yCoords[timeii][nonAdjCellID],
                                            ]
                                        )
                    ...
                    histEdgeLength.hist(
                        edgeLengthsList, bins=20, color="blue", alpha=0.7
                    )
                    histEdgeLength.set_xlabel("Edge length (\sqrt(a_0/\pi)")
                    ...
                    histLifetime.set_xlabel("Neighbor lifetime (min)")
                    histLifetime.set_ylabel("Counts")
                    print(
                        "Neighbors with lifetime 1 frame:",
                        neighborLifetimesList[0],
                        "Neighbors with lifetime >1 frame:",
                        sum(neighborLifetimesList[1:]),
                    )
                    histSeparation = histFig.add_subplot(2, 2, 3)
                    histSeparation.hist(
                        deadNeighborSeparationList, bins=20, color="blue", alpha=0.7
                    )
                    histSeparation.set_xlabel("Neighbor separation (\sqrt(a_0/\pi)")
                    ...
                    for timeii in range(0, len(cellPos)):
                        if timeii > 0 and timeii < len(cellPos) - persistThreshold:
                            ...
                            neighborDifferences = {
                                k: cellNeighborList[timeii][k]
                                - cellNeighborList[timeii - 1][k]
                                for k in cellNeighborList[timeii]
                            }
                            # for each cell k,
                            for k in neighborDifferences:
                                neighborCounter += 1
                                if all(
                                    element >= 0 for element in neighborDifferences[k]
                                ):
                                    neighborCounter2 += 1
                                    ...
                                    if neighborDifferences[k] and any(
                                        neighborDifferences[l]
                                        for l in neighborDifferences[k]
                                    ):
                                        ...
                                        for l in range(1, persistThreshold + 1):
                                            ...
                                            isFutureNeighbor = bool(
                                                neighborDifferences[k]
                                                & cellNeighborList[timeii + l][k]
                                            )
                                            doesNeighborPersist = (
                                                doesNeighborPersist and isFutureNeighbor
                                            )
                                        if doesNeighborPersist:
                                            neighborExchangeCount[timeii] += 1
                                            print(
                                                "neighbor exchange between cells",
                                                k,
                                                ",",
                                                neighborDifferences[k],
                                                "at frame:",
                    ...
                    def make_animation_step(frame):
                        cells = pyvoro.compute_2d_voronoi(
                            np.column_stack((xCoords[frame, :], yCoords[frame, :])),
                            [
                                [
                                    min(xCoords[frame, :]) - 1,
                                    max(xCoords[frame, :]) + 1,
                        ...
                        tessAxs.clear()
                        time_text = tessAxs.text(0.05, 0.95, "")
                    ...
                    ani2.save("tessAnimation2.gif", writer="pillow", fps=100, dpi=50)
                    plt.show()
